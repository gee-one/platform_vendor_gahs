...
function setup_vendor() {
    ...
    export GAHS_ROOT="$3"
    if [ ! -d "$GAHS_ROOT" ]; then
        echo "\$GAHS_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$GAHS_ROOT/$OUTDIR" ]; then
        mkdir -p "$GAHS_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$GAHS_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$GAHS_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$GAHS_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$GAHS_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$GAHS_ROOT"/prebuilts/tools-extra/common/smali/baksmali.jar
        export SMALIJAR="$GAHS_ROOT"/prebuilts/tools-extra/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$GAHS_ROOT"/prebuilts/tools-extra/"${HOST,,}"-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$GAHS_ROOT"/prebuilts/tools-extra/"${HOST,,}"-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$GAHS_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$GAHS_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$GAHS_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$GAHS_ROOT"/vendor/gahs/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$GAHS_ROOT"/"$OUTDIR"/radio
